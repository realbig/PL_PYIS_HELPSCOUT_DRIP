...
class PYIS_HelpScout_Drip_REST {
	...
	private function build_response_html() {
		...
		if ( property_exists( $this->drip_data, 'errors' ) ) {
			...
			return '<div class="toggleGroup"><p>' . sprintf( _x( '<a href="mailto:%s">%s</a> does not exist in Drip. Maybe the Drip credentials are incorrect?', 'Email Address does not exist in Drip', PYIS_HelpScout_Drip_ID ), $subscriber_email, $subscriber_email ) . '</p></div><div class="divider"></div>';
